(() => {
  ...
  const optionMatches = (element, value) => normalize(element.textContent) === normalize(value) || normalize(element.getAttribute("value")) === normalize(value);
  ...
  const overlayCandidates = () => {
    ...
    const candidates = roots.flatMap(root => [...root.querySelectorAll("[role='option'], [role='menuitem'], mat-option, li, button, option, .mat-option, .mat-mdc-option, .mdc-list-item, div, span")]);
    return [...new Set(candidates)].filter(visible).filter(element => ![...element.children].some(child => normalize(child.textContent) === normalize(element.textContent)));
  };
  ...
  const setCustomSelect = async (element, value) => {
    ...
    let option = null;
    for (let attempt = 0; attempt < 30 && !option; attempt++) { option = overlayCandidates().find(item => optionMatches(item, value)); if (!option) await wait(50); }
    if (!option) { element.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", bubbles: true })); return false; }
    option.dispatchEvent(new MouseEvent("mousedown", { bubbles: true, composed: true })); option.click(); option.dispatchEvent(new MouseEvent("mouseup", { bubbles: true, composed: true })); await wait(150); return true;
  };

  const best = (available, keywords, used) => {
    let selected = null; let score = 0;
  ...
  const fillGeneralDetails = async details => {
    const available = fields(document); const used = new Set(); let count = 0;
    const mappings = [
    ...
    for (const [keywords, key] of mappings) { if (setValue(best(available, keywords, used), details?.[key])) count++; }
    return count;
  };

  const fillPilgrim = async (root, pilgrim) => {
    const available = fields(root); const used = new Set(); let count = 0;
    ...
    if (setValue(name, pilgrim.name)) count++; if (setValue(age, pilgrim.age)) count++; if (await setCustomSelect(gender, pilgrim.gender)) count++; if (await setCustomSelect(proof, pilgrim.idType)) count++; if (setValue(number, pilgrim.idNumber)) count++;
    return count;
  };

  const findGroups = expected => {
    const all = fields(document);
    const candidates = [...document.querySelectorAll("fieldset, section, article, li, tr, .row, [class*='pilgrim'], [class*='passenger'], [class*='devotee']")].filter(visible).filter(group => fields(group).length >= 3);
    const unique = [];
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    (async () => {
      try {
        ...
        const general = await fillGeneralDetails(message.profile?.generalDetails || {});
        ...
        sendResponse({ message: filled ? `Filled ${filled} field(s), including general details. Review all details before continuing.` : "No matching fields found. Check the page and form fields." });
